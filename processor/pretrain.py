...
from SA_SOiCLR.net.losses import SupConLoss
...
class PT_Processor(Processor):
    ...
    def load_model(self):
        self.model = self.io.load_model(self.arg.model,  # 实例化图网络模型，self.arg.model=net.aimclr.AimCLR，也就是实例化banckbone,projector，以及部分算法
                                        **(self.arg.model_args))
        self.model.apply(weights_init)
        self.CEloss = nn.CrossEntropyLoss()
        self.loss = SupConLoss()
        self.re_criterion = torch.nn.L1Loss(reduction='none')  # 不知道对比学习为啥要l1loss
