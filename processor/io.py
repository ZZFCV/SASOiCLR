...
class IO():
    ...
    def load_arg(self, argv=None):
        ...
        p = parser.parse_args(argv)  # 把--config读入parser里，并返回p，那么p就包含了--config以及前面设置的所有参数
        if p.config is not None:
            # load config file
            with open(p.config, 'r') as f:
                default_arg = yaml.load(f, Loader=yaml.FullLoader)  # 读取yaml配置文件
    ...
    def init_environment(self):
        self.io = torchlight.IO(  # 设置日志、输出等文件保存路径
            self.arg.work_dir,
