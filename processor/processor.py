...
class Processor(IO):
    ...
    def __init__(self, argv=None):  # argv是个列表，里面两个元素，分别是--config,config/ntu60_frame50/pretext/pretext_aimclr_xview_joint

        self.load_arg(argv)
        self.init_environment()  # 初始化环境，比如说设置gpu等等

        if self.arg.phase == 'train':
            if os.path.isdir(self.arg.work_dir + '/train'):  # 好像是用来写日志的目录
        ...
        self.load_model()  # 去到其子类pretrain.py下的PT_Processor进行初始化,实例化模型
        self.load_weights()  # 预训练阶段不用load_weights，在线性探针阶段就要把自己的权重导入了
        self.gpu()  # 把模型和损失啥的都丢进去
    ...
    def train_log_writer(self, epoch):
        with open(self.arg.work_dir + 'log.txt', mode='at', encoding='utf-8') as f:
            f.write('batch_loss = {}, lr = {}, epoch = {}'.format(self.iter_info['loss'],self.lr,epoch))
    ...
    def init_environment(self):
        super().init_environment()   # 去设置gpu号以及输出路径
        self.result = dict()
        self.iter_info = dict()
    ...
    def load_data(self):
        ...
        if self.arg.train_feeder_args:  # self.arg.train_feeder_args包括了数据集地址，label地址，以及数据增强的参数
            train_feeder = import_class(self.arg.train_feeder)  # train_feeder = 'feeder.ntu_feeder.Feeder_triple'
    ...
    def start(self):
        ...
        # training phase
        if self.arg.phase == 'train':
            self.global_step = self.arg.start_epoch * len(self.data_loader['train'])  # 这个应该就是iter次数
            self.meta_info['iter'] = self.global_step
            self.best_result = 0.0
            
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):  # 预训练训练300轮，线性探针训练100轮
                ...
                if self.arg.save_interval == -1:
                    pass
                elif ((epoch + 1) % self.arg.save_interval == 0) or (  # 预训练阶段就走这一步，所以预训练阶段不包括validation
                ...
                if self.arg.eval_interval == -1:
                    pass
                elif ((epoch + 1) % self.arg.eval_interval == 0) or (  # 每五轮做一次validation操作
                        epoch + 1 == self.arg.num_epoch):
                    self.io.print_log('Eval epoch: {}'.format(epoch + 1))
                    self.test(epoch + 1)  # 做validation
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--save_result', type=str2bool, default=True, help='if ture, the output of the model will be stored')  # 就是一个bool类型
