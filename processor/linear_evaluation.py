...
class LE_Processor(Processor):
    ...
    def load_model(self):
        self.model = self.io.load_model(self.arg.model,  # 实例化aimclr类
                                        **(self.arg.model_args))
        self.model.apply(weights_init)

        for name, param in self.model.encoder_q.named_parameters():  # 冻结所有层，除了最后一层
    ...
    def show_topk(self, k):
        rank = self.result.argsort()  # argsott()只返回索引
    ...
    def train(self, epoch):
        self.model.eval()  # 在线性探针的训练阶段也得用eval模式
        self.adjust_lr()
        loader = self.data_loader['train']
        loss_value = []

        for data, label in loader:  # data.shape=[128,3,50,25,2]
            self.global_step += 1
            # get data
            ...
            # forward
            output = self.model(data)  # 就走最简单的一个前向传播了
    ...
    def test(self, epoch):
        ...
        for data, label in loader:  # 注意即使是在做validation，也要有数据增强。
            # get data
            data = data.float().to(self.dev, non_blocking=True)
            label = label.long().to(self.dev, non_blocking=True)
        ...
        # show top-k accuracy 
        for k in self.arg.show_topk:  # 计算topk准确率
            self.show_topk(k)
        self.show_best(1)  # 计算top1准确率，若超过之前，就更新
